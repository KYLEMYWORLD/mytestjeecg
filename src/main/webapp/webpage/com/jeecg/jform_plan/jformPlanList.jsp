...
<div class="easyui-layout" fit="true">
    <div region="center" style="padding:0px;border:0px">
        <t:datagrid name="jformPlanList" checkbox="false" pagination="true" treegrid="true" treeField="planName"
                    fitColumns="true" title="项目任务计划表" sortName="createDate" actionUrl="jformPlanController.do?datagrid"
                    idField="id" fit="true" queryMode="group">
            <t:dgCol title="主键" field="id" hidden="true" queryMode="single" width="120"></t:dgCol>
            <t:dgCol title="项目名称" field="projectId" query="true" queryMode="single" width="60" dictionary="jform_project,id,project_name" searchpopdic="project_select,projectId@cname,id@project_name"></t:dgCol>
            ...
            <t:dgDelOpt id="delete" exp="planStatus#eq#0,1" title="删除" url="jformPlanController.do?doDel&id={id}"
                        urlclass="ace_button" urlfont="fa-trash-o"/>
            <t:dgFunOpt id="sendapprove" exp="planLevel#eq#1&&planStatus#eq#0,4" funname="doSendApprove(id,index)"
                        title="提交审批" urlclass="ace_button" urlfont="fa-wrench"/>
            <t:dgFunOpt id="doapprove" exp="planLevel#eq#1&&planStatus#eq#3" funname="doApprove(id,index)" title="审批"
                        urlclass="ace_button" urlfont="fa-wrench"/>
            <t:dgFunOpt id="approvemsg" exp="planLevel#eq#1&&planStatus#eq#4" funname="doApproveMsg(id,index)"
                        title="查看审批信息" urlclass="ace_button" urlfont="fa-wrench"/>
            <t:dgFunOpt id="senddivide" exp="planLevel#eq#2&&planStatus#eq#0" funname="doSendDivide(id)" title="下发细分"
                        urlclass="ace_button" urlfont="fa-wrench"/>
            <t:dgFunOpt id="dividefinish" exp="planLevel#eq#2&&planStatus#eq#1" funname="doDivideFinish(id)"
                        title="细分完成" urlclass="ace_button" urlfont="fa-wrench"/>
            <t:dgToolBar id="addtask" title="添加分配任务" icon="icon-add" url="jformPlanController.do?goAddFirst" funname="BeforeFirstAdd" width="635" height="470"></t:dgToolBar>
            <t:dgToolBar id="add" title="添加任务" icon="icon-add" url="jformPlanController.do?goAdd" funname="BeforeAdd" width="635" height="470"></t:dgToolBar>
            <t:dgToolBar id="update" title="编辑" icon="icon-edit" url="jformPlanController.do?goUpdate" width="635" height="470" funname="updatetree"></t:dgToolBar>
            <t:dgToolBar id="detail" title="查看" icon="icon-search" url="jformPlanController.do?goUpdate" funname="detailtree" width="635" height="470"></t:dgToolBar>
