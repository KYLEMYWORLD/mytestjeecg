...
<#else>
 ...
 />
   ...
   <t:dgToolBar id="add" title="录入" icon="icon-add" url="${entityName?uncap_first}Controller.do?goAdd" funname="add"></t:dgToolBar>
   <#--//update-begin--Author:gj_shaojc  Date:20180402 for：TASK #2606 【代码生成器】树形列表生成，多选处理 -->
   <#if cgformConfig.cgFormHead.isTree == 'Y'>
   <t:dgToolBar id="update" title="编辑" icon="icon-edit" url="${entityName?uncap_first}Controller.do?goUpdate" funname="updatetree" width="100%" height="100%"></t:dgToolBar>
   <t:dgToolBar id="deleteselect" title="批量删除"  icon="icon-remove" url="${entityName?uncap_first}Controller.do?doBatchDel" funname="deleteALLSelecttree"></t:dgToolBar>
   <t:dgToolBar id="detail" title="查看" icon="icon-search" url="${entityName?uncap_first}Controller.do?goUpdate" funname="detailtree" width="100%" height="100%"></t:dgToolBar>
   <#else>
   ...
   <#list buttons as btn>
    ...
    <#if btn.optType == 'action'>
     	<t:dgToolBar id="${btn.buttonCode}" title="${btn.buttonName}" icon="${btn.buttonIcon}" <#if btn.optType=='action'> url="${entityName?uncap_first}Controller.do?do${btn.buttonCode?cap_first}" funname="do${btn.buttonCode?cap_first}"<#else> funname="${btn.buttonCode}"</#if> ></t:dgToolBar>
	<#else>
		<t:dgToolBar id="${btn.buttonCode}" title="${btn.buttonName}" icon="${btn.buttonIcon}" funname="do${btn.buttonCode?cap_first}"></t:dgToolBar>
	</#if>
   	</#if>
   </#list> 
  </t:datagrid>
